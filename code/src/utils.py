...
def models_load(args, data):
    ...
    if args.model=='FM':
        ...
        model = StackedDeepCrossNetworkModel(args, data).to(args.device)
    elif args.model=='PDCN':
        model = ParallelDeepCrossNetworkModel(args, data).to(args.device)
    elif args.model=='CNN_FM':
        model = CNN_FM(args, data).to(args.device)
    elif args.model=='DeepCoNN':
        model = DeepCoNN(args, data).to(args.device)
    else:
        raise ValueError('MODEL is not exist : select model in [FM,FFM,NCF,WDN,SDCN,PDCN,CNN_FM,DeepCoNN]')
    return model
